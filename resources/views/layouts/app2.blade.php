        ...
        <div class="border-right" id="sidebar-wrapper" style='background-color:	#139C18;color:white;height:100vh;'>
            <div class="sidebar-heading" style='text-align:center;padding:0px;height:35%;'>
                <div class='background-image-header'>
                    ...
                    <img src="{{'http://10.96.4.126:8668/storage/id_image/employee_image/'.$name->id.'.png'}}" class="rounded-circle"  style='height:150px;border:1px white solid;background-color:white; ' alt="Cinque Terre">
            ...
            <div class="list-group list-group-flush overflow-auto" style='max-height:60vh;min-height:60vh;'>
                <span class="list-group-item list-group-item-action " style='color:white;background-color:black;'>Heads Up!</span>
                
                @foreach($upcomming_eventsa as $key => $upcoming_eventa)
                    @php
                    $color = $color - 18;
                    @endphp 
                    
                        <a href="#" class="list-group-item-a"  href="#{{$color}}" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle" style='background-color: rgb({{$color}},{{$color}},{{$color}});'>{{str_limit($upcoming_eventa->title,20)}}<br>
                        <p id="side_bar_countdown{{$key}}"></p></a>
                        <input id='side_bar_countdown{{$key}}_1' type='hidden' value='{{date('M d, Y H:i:s',strtotime($upcoming_eventa->date_happen))}}'>
                      
                        
                        <script>
                            
                            // Set the date we're counting down to
                            var demo{{$key}} = "side_bar_countdown"+{{$key}};
                            var demo_1{{$key}} = "side_bar_countdown"+{{$key}}+"_1";
                            
                            var number{{$key}} = document.getElementById(demo_1{{$key}}).value;
                          
                            var countDownDate{{$key}} = new Date(number{{$key}}).getTime();
